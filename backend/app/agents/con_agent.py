...
from app.prompts import con_prompts


class ConAgent(BaseAgent):
    async def opening(self, topic: str, model_name: str | None = None) -> str:
        return await self._generate(
            system_prompt=con_prompts.SYSTEM_PROMPT,
            user_prompt=con_prompts.opening_prompt(topic),
    ...
    async def rebuttal(
        ...
        return await self._generate(
            system_prompt=con_prompts.SYSTEM_PROMPT,
            user_prompt=con_prompts.rebuttal_prompt(topic, own_opening, opponent_opening),
            model_name=model_name,
        )
    ...
    async def closing(
        ...
        return await self._generate(
            system_prompt=con_prompts.SYSTEM_PROMPT,
            user_prompt=con_prompts.closing_prompt(
                topic,
                own_opening,
                opponent_opening,
                own_rebuttal,
